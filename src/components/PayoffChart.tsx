...
import { buildPriceRange, computePayoffAtExpiry, hasMixedExpiries } from "../lib/payoff";
...
export function PayoffChart({
  ...
  /** Estruturas convexas (ratio backspread) têm ganho sem teto — mostra um aviso em vez de sugerir um limite falso. */
  ...
  if (hasMixedExpiries(legs)) {
    return (
      <div
        className="flex items-center justify-center text-center text-xs text-[var(--color-muted)] px-4"
        style={{ height }}
      >
        Gráfico de payoff não disponível: as pernas têm vencimentos diferentes (estrutura de
        calendário). O resultado no vencimento curto depende do valor a mercado da perna de
        vencimento longo naquela data, que exige um modelo de precificação — não apenas o
        valor intrínseco no vencimento que este gráfico usa.
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      {maxGainUnlimited && (
        <p className="text-[10px] text-[var(--color-muted)] text-center -mt-1">
          Ganho sem teto além da borda direita do gráfico — a linha continua subindo.
        </p>
      )}
    </div>
  );
